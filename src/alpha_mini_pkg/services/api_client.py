# src/alpha_mini_pkg/services/api_client.py
...
# --- Оголошення Типів для чистоти коду ---
# Використовуємо ResultType, оскільки це важливо для перевірки
ApiResult = tuple[MiniApiResultType, WiFiDevice | object] 


# --- Низькорівневі Обгортки для Дій (Actions) ---

async def move_robot(steps: int, direction: MoveRobotDirection) -> bool:
    """
    Переміщує робота на задану кількість кроків у заданому напрямку.

    :param steps: Кількість кроків.
    :param direction: Напрямок руху (FORWARD, BACKWARD, etc.).
    :return: True, якщо рух успішний, інакше False.
    """
...
async def start_tts(text: str) -> bool:
    """
    Запускає синтез мовлення (Text-To-Speech).

    :param text: Текст для озвучення.
    :return: True, якщо TTS успішно запущено, інакше False.
    """
    ...
    # ПРИМІТКА: Цей API лише підтверджує ЗАПУСК TTS, а не завершення.
    if result_type == MiniApiResultType.Success and response.isSuccess:
...
async def play_named_action(action_name: str) -> bool:
    """
    Виконує одну попередньо задану дію за її назвою.
    
    :param action_name: Назва дії (наприклад, "Welcome").
    :return: True, якщо дія успішно запущена.
    """
...
# --- Низькорівнева Обгортка для Спостереження (Observe) ---

def create_speech_observer() -> ObserveSpeechRecognise:
    """
    Створює та повертає об'єкт ObserveSpeechRecognise.
    Це дозволяє подальшій логіці (listeners) встановити обробник.

    :return: Об'єкт ObserveSpeechRecognise.
    """
